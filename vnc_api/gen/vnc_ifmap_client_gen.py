...
class VncIfmapClientGen(object):
    ...
    def _ifmap_global_vrouter_config_set(self, my_imid, existing_metas, obj_dict):
        ...
        if field is not None:
            norm_str = escape(str(obj_dict['forwarding_mode']))
            meta = Metadata('forwarding-mode', norm_str,
                   {'ifmap-cardinality':'singleValue'}, ns_prefix = 'contrail')

            if (existing_metas and 'forwarding-mode' in existing_metas and
                str(existing_metas['forwarding-mode'][0]['meta']) == str(meta)):
                # no change
                pass
            else:
                self._update_id_self_meta(update, meta)
    ...
    def _ifmap_global_vrouter_config_read_to_meta_index(self, ifmap_id, field_names = None):
        ...
        all_metas = [u'linklocal-services', u'encapsulation-priorities', u'vxlan-network-identifier-mode', u'forwarding-mode', u'id-perms', u'display-name']
        if not field_names:
            metas_to_read = all_metas
        else: # read only requested fields
            metas_to_read = set(all_metas) & set(field_names.keys())
    ...
    def _ifmap_global_vrouter_config_update(self, ifmap_id, new_obj_dict):
        ...
        # remove properties that are no longer active
        props = ['linklocal-services', 'encapsulation-priorities', 'vxlan-network-identifier-mode', 'forwarding-mode', 'id-perms', 'display-name']
        for prop in props:
            prop_m = prop.replace('-', '_')
            if prop in existing_metas and prop_m not in new_obj_dict:
                self._delete_id_self_meta(ifmap_id, 'contrail:'+prop)
    ...
    def _ifmap_instance_ip_set(self, my_imid, existing_metas, obj_dict):
        ...
        if field is not None:
            norm_str = escape(str(obj_dict['instance_ip_secondary']))
            meta = Metadata('instance-ip-secondary', norm_str,
                   {'ifmap-cardinality':'singleValue'}, ns_prefix = 'contrail')

            if (existing_metas and 'instance-ip-secondary' in existing_metas and
                str(existing_metas['instance-ip-secondary'][0]['meta']) == str(meta)):
                # no change
                pass
            else:
                self._update_id_self_meta(update, meta)
    ...
    def _ifmap_instance_ip_read_to_meta_index(self, ifmap_id, field_names = None):
        ...
        all_metas = [u'instance-ip-virtual-network', u'instance-ip-virtual-machine-interface', u'instance-ip-address', u'instance-ip-family', u'instance-ip-mode', u'subnet-uuid', u'instance-ip-secondary', u'id-perms', u'display-name']
        if not field_names:
            metas_to_read = all_metas
        else: # read only requested fields
            metas_to_read = set(all_metas) & set(field_names.keys())
    ...
    def _ifmap_instance_ip_update(self, ifmap_id, new_obj_dict):
        ...
        # remove properties that are no longer active
        props = ['instance-ip-address', 'instance-ip-family', 'instance-ip-mode', 'subnet-uuid', 'instance-ip-secondary', 'id-perms', 'display-name']
        for prop in props:
            prop_m = prop.replace('-', '_')
            if prop in existing_metas and prop_m not in new_obj_dict:
                self._delete_id_self_meta(ifmap_id, 'contrail:'+prop)
    ...
    def _ifmap_loadbalancer_pool_set(self, my_imid, existing_metas, obj_dict):
        ...
        if field is not None:
            # construct object of xsd-type and get its xml repr
            field = obj_dict['loadbalancer_pool_custom_attributes']
            buf = cStringIO.StringIO()
            # perms might be inserted at server as obj.
            # obj construction diff from dict construction.
            if isinstance(field, dict):
                KeyValuePairs(**field).exportChildren(buf, level = 1, name_ = 'loadbalancer-pool-custom-attributes', pretty_print = False)
            else: # object
                field.exportChildren(buf, level = 1, name_ = 'loadbalancer-pool-custom-attributes', pretty_print = False)
            loadbalancer_pool_custom_attributes_xml = buf.getvalue()
            buf.close()
            meta = Metadata('loadbalancer-pool-custom-attributes' , '',
                   {'ifmap-cardinality':'singleValue'}, ns_prefix = 'contrail',
                   elements = loadbalancer_pool_custom_attributes_xml)

            if (existing_metas and 'loadbalancer-pool-custom-attributes' in existing_metas and
                str(existing_metas['loadbalancer-pool-custom-attributes'][0]['meta']) == str(meta)):
                # no change
                pass
            else:
                self._update_id_self_meta(update, meta)
    ...
    def _ifmap_loadbalancer_pool_read_to_meta_index(self, ifmap_id, field_names = None):
        ...
        all_metas = [u'loadbalancer-pool-service-instance', u'loadbalancer-pool-virtual-machine-interface', u'loadbalancer-pool-service-appliance-set', u'loadbalancer-pool-loadbalancer-healthmonitor', u'virtual-ip-loadbalancer-pool', u'loadbalancer-pool-properties', u'loadbalancer-pool-provider', u'loadbalancer-pool-custom-attributes', u'id-perms', u'display-name', u'loadbalancer-pool-loadbalancer-member']
        if not field_names:
            metas_to_read = all_metas
        else: # read only requested fields
            metas_to_read = set(all_metas) & set(field_names.keys())
    ...
    def _ifmap_loadbalancer_pool_update(self, ifmap_id, new_obj_dict):
        ...
        # remove properties that are no longer active
        props = ['loadbalancer-pool-properties', 'loadbalancer-pool-provider', 'loadbalancer-pool-custom-attributes', 'id-perms', 'display-name']
        for prop in props:
            prop_m = prop.replace('-', '_')
            if prop in existing_metas and prop_m not in new_obj_dict:
                self._delete_id_self_meta(ifmap_id, 'contrail:'+prop)
